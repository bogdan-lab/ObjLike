...
def test_plane_creation():
    ...
    assert sum(np.isclose(p.x, -1) for p in pl.description.points) == 2
    assert sum(np.isclose(p.x, 1) for p in pl.description.points) == 2
...
def test_tube_creation_1_layer():
    ...
    assert sum(np.isclose(p.z, 0) for p in tube.description.points) == 6
...
def test_tube_creation_3_layer():
    ...
    assert sum(np.isclose(p.z, 0) for p in tube.description.points) == 6
    assert sum(np.isclose(p.z, 3) for p in tube.description.points) == 6
    assert sum(np.isclose(p.z, 6) for p in tube.description.points) == 6
    assert sum(np.isclose(p.z, 9) for p in tube.description.points) == 6
    assert sum(np.isclose(p.r, 5) for p in tube.description.points) == 6
...
def test_tube_creation_2_layer():
    ...
    assert sum(np.isclose(p.z, 0) for p in tube.description.points) == 12
    assert sum(np.isclose(p.z, 3) for p in tube.description.points) == 12
    assert sum(np.isclose(p.z, 6) for p in tube.description.points) == 12
...
def test_cylinder_creation_1_layer():
    ...
    assert sum(np.isclose(p.z, 0) for p in c.description.points) == 7
    assert sum(np.isclose(p.z, 2) for p in c.description.points) == 7
...
def test_cylinder_creation_2_layer():
    ...
    assert sum(np.isclose(p.z, 0) for p in c.description.points) == 19
    assert sum(np.isclose(p.z, 3) for p in c.description.points) == 12
    assert sum(np.isclose(p.z, 6) for p in c.description.points) == 19
    assert Point(0, 0, 0) in c.description.points
    assert sum(np.isclose(p.r, 2) for p in c.description.points) == 6
    assert sum(np.isclose(p.r, 4) for p in c.description.points) == 12
    assert sum(np.isclose(p.r, 5) for p in c.description.points) == 12
    assert Point(0, 0, 6) in c.description.points
    assert sum(np.isclose(p.r, np.sqrt(40)) for p in c.description.points) == 6
...
def test_cone_creation_1_layer():
    ...
    assert sum(np.isclose(p.z, 0) for p in cone.description.points) == 7
...
def test_cone_creation_3_layer():
    ...
    assert sum(np.isclose(p.z, 0) for p in cone.description.points) == 37
    assert Point(0, 0, 0) in cone.description.points
    assert Point(0, 0, 18) in cone.description.points
    assert sum(np.isclose(p.r, 3) for p in cone.description.points) == 6
    assert sum(np.isclose(p.r, 6) for p in cone.description.points) == 12
...
def test_cone_no_base_creation_3_layer():
    ...
    assert sum(np.isclose(p.z, 0) for p in cone.description.points) == 18
    assert Point(0, 0, 18) in cone.description.points
    assert sum(np.isclose(p.r, 9) for p in cone.description.points) == 18
    assert sum(np.isclose(p.z, 6) for p in cone.description.points) == 12
    assert sum(np.isclose(p.z, 12) for p in cone.description.points) == 6
...
def test_box_creation():
    ...
    assert sum(np.isclose(p.x, -1) for p in box.description.points) == 4
    assert sum(np.isclose(p.x, 1) for p in box.description.points) == 4
...
def test_world_add_object():
    ...
    assert sum(np.isclose(p.z, 0) for p in world.description.points) == 4
    assert sum(np.isclose(p.z, 10) for p in world.description.points) == 4
    assert sum(np.isclose(p.z, 2.5) for p in pl1.description.points) == 2
    assert sum(np.isclose(p.z, -2.5) for p in pl1.description.points) == 2
